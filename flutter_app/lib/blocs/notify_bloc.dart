...
class NotifyBloc extends Bloc<NotifyEvent, NotifyState> {
  ...
  NotifyBloc({this.appRepository})
      : assert(appRepository != null),
        super(NotifyStateInitial());
  ...
  Stream<NotifyState> mapEventToState(NotifyEvent notifyEvent) async* {
    ...
    if (notifyEvent is NotifyManagerEventRequested) {
      ...
      try {
        final ResponseNotify response = await new Future.delayed(
      ...
      yield NotifyStateLoading();
      try {
        final ResponseNotifyCM response = await new Future.delayed(
            const Duration(milliseconds: Constant.duration), () {
